#pragma once
#ifndef __HALAK_STRING_INL__
#define __HALAK_STRING_INL__
...
#   include <stdexcept>
    ...
    namespace Halak
    {
        ...
        String::String(const char* s)
            : buffer((s && s[0] != '\0') ? shared_ptr<StringBuffer>(new StringBuffer(s)) : Empty.buffer)
        {
        }

        String::String(const std::string& s)
            : buffer((s.empty() == false) ? shared_ptr<StringBuffer>(new StringBuffer(s.data(), static_cast<int>(s.size()))) : Empty.buffer)
        {
        }
