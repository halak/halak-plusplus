...
namespace Halak
{
        ...
        struct ParserCallback
        {
            ...
            static void PushValue(ReadingContext* context, const Any& value)
            {
                if (context->Key.IsEmpty())
                    static_pointer_cast<JSONArray>(context->Nodes.top())->Add(value);
                else
                    static_pointer_cast<JSONObject>(context->Nodes.top())->Add(context->Key, value);
    ...
    JSONParsingError::JSONParsingError(int errorCode, const String& filename, int line, int column)
        : std::exception(JSONParsingErrorToString(errorCode)),
          Filename(filename),
          Line(line),
          Column(column)
    {
